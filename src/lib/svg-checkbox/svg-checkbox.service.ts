/*
 * Author: Mary Lou (https://tympanus.net/codrops/author/crnacura/)
 * Date: 15-Oct-2013
 * Source URL: https://tympanus.net/codrops/2013/10/15/animated-checkboxes-and-radio-buttons-with-svg/
 * Modified by: Yuvraj Patil
*/
...
export class SvgCheckboxService {
  ...
  drawSVGCheckbox() {
    if (document.createElement('svg').getAttributeNS ) {
      ...
      function controlCheckbox( el, type ) {
        var svg = createSVGEl(undefined);
        el.parentNode.appendChild( svg );

        var changeCheckboxValue = function(el) {
          if( el.checked ) {
            draw( el, type );
          }
          else {
